...
class Parser:
    ...
    def sentence(self) -> list:
        try:
            return [(s['sentence'], s['sentence-translation'],) for s in
                    self._result['blng_sents_part']['sentence-pair']]
        except KeyError:
            return []
...
class API():
    ...
    params = {"dicts": {"count": 99,
                        "dicts": [["ec", "ee", "phrs", "pic_dict"], ["web_trans"], ["fanyi"], ["blng_sents_part"]]}}
    ...
    def query(cls, word) -> dict:
        ...
        try:
            ...
            # print(f'code:{rsp.status_code}- word:{word} text:{rsp.text}')
            # print(rsp.json())
